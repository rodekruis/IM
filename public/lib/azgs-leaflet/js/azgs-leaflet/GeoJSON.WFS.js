L.GeoJSON.WFS = L.GeoJSON.extend({
	...
	onAdd: function(map) {
		...
		this.getFeature(function() {
			L.geoJson(that.jsonData).addTo(map);
		});
	},
	...
	getFeature: function(callback) {
		var that = this;
		/*
		var geojsonLayer = L.geoJson.ajax(that.options.proxy + encodeURIComponent(this.getFeatureUrl), 
			function(data){
			    that.jsonData = data;
		...
		*/
		$.ajax({
			url: that.options.proxy + encodeURIComponent(this.getFeatureUrl),
			type: "GET",
			success: function(response) {
				var res = JSON.parse(response);
				if (res.type && res.type == "FeatureCollection") {
					that.jsonData = res;
					that.toGeographicCoords(that.options.inputCrs || "EPSG:900913");
					callback();
				}				
